...
class UserBase:
    ...
    def role_limited_admin(self):
        return self._has_role(constants.ROLE_LIMITED_ADMIN)
...
class User(UserBase, Base):
    ...
    sidebar_view = Column(Integer, default=1 | (1 << 1) | (1 << 18) | (1 << 19))  # Include Language, Series Tracker, Author Dashboard
...
class UserPreference(Base):
    ...
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey('user.id'))
    item_type = Column(SmallInteger, nullable=False)  # 0=Book, 1=Series, 2=Author
    item_id = Column(Integer, nullable=False)
    status = Column(SmallInteger, nullable=False)  # 1=Preferred, -1=Ignored
    last_modified = Column(DateTime, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))

    def __repr__(self):
        return '<UserPreference uid:%d type:%d iid:%d status:%d>' % (self.user_id, self.item_type, self.item_id, self.status)


class AccessRequest(Base):
    """Pending phpBB user access requests awaiting admin approval."""
    __tablename__ = 'access_request'

    id = Column(Integer, primary_key=True)
    phpbb_user_id = Column(Integer, nullable=False, unique=True)
    username = Column(String, nullable=False)
    email = Column(String, nullable=False)
    requested_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
    status = Column(SmallInteger, default=0)  # 0=pending, 1=approved, -1=rejected

    def __repr__(self):
        return '<AccessRequest %s (phpBB ID: %d) status:%d>' % (self.username, self.phpbb_user_id, self.status)


class RejectedAccess(Base):
    """Tracks rejected access requests with cooldown and blocking after 3 strikes."""
    __tablename__ = 'rejected_access'

    id = Column(Integer, primary_key=True)
    phpbb_user_id = Column(Integer, nullable=False, unique=True)
    username = Column(String, nullable=False)
    rejection_count = Column(Integer, default=1)
    last_rejection_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
    rejection_reason = Column(String, nullable=True)
    blocked = Column(Boolean, default=False)  # True after 3+ rejections

    def __repr__(self):
        return '<RejectedAccess %s (%d rejections, blocked:%s)>' % (self.username, self.rejection_count, self.blocked)
...
class BookHealth(Base):
    ...
    has_azw = Column(Boolean, default=False)
    has_epub = Column(Boolean, default=False)
    has_docx_cz = Column(Boolean, default=False)
    extra_formats = Column(JSON, default=[])
    desc_lang = Column(String(3), default="")
    last_scan = Column(DateTime, default=lambda: datetime.now(timezone.utc))

    def __repr__(self):
        return '<BookHealth book_id:%d healthy:%s>' % (self.book_id, self.is_healthy)
...
def migrate_read_book_table(engine, _session):
    try:
        _session.query(exists().where(ReadBook.progress_percent)).scalar()
        _session.commit()
...
def promote_admin(username):
    ...
    if user:
        from . import roles
        user.role = roles.ROLE_FULL_ADMIN
        try:
            session.commit()
            print("User '{}' promoted to Full Admin".format(username))
        except Exception as e:
            session.rollback()
            print("Critical error: Failed to promote user: {}".format(e))
    else:
