...
def opds_download_link(book_id, book_format):
    ...
    client = ""
    # Identification of specific e-reader apps and devices
    client_map = {
        "Kobo": "kobo",
        "Kindle": "kindle",
...
@opds.route("/opds/progress/<int:book_id>")
@requires_basic_auth_if_no_ano
def opds_update_progress(book_id):
    # Standard OPDS-PSE progress update endpoint
    # Parameters: pc (percentage), st (status: finished, in_progress, unread)
    pc = request.args.get("pc", type=float)
    st = request.args.get("st", type=str)
    
    if pc is None and st is None:
        return jsonify({"status": "error", "message": "No parameters provided"}), 400
        
    read_book = ub.session.query(ub.ReadBook).filter(
        ub.ReadBook.user_id == int(auth.current_user().id),
        ub.ReadBook.book_id == book_id
    ).first()
    
    if not read_book:
        read_book = ub.ReadBook(user_id=int(auth.current_user().id), book_id=book_id)
        ub.session.add(read_book)
        
    if pc is not None:
        read_book.progress_percent = max(0.0, min(100.0, pc))
        if pc >= 99.0:
    ...
    if st:
        if st == "finished":
            read_book.read_status = ub.ReadBook.STATUS_FINISHED
            read_book.progress_percent = 100.0
        elif st == "in_progress":
            read_book.read_status = ub.ReadBook.STATUS_IN_PROGRESS
        elif st == "unread":
            read_book.read_status = ub.ReadBook.STATUS_UNREAD
            read_book.progress_percent = 0.0
            
    try:
        ub.session.commit()
        return jsonify({"status": "ok", "progress": read_book.progress_percent, "read_status": read_book.read_status})
    except Exception as e:
        ub.session.rollback()
        log.error("Failed to update OPDS progress: {}".format(e))
