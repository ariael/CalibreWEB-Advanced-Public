...
$(document).ready(function () {
    $(document).on('click', '.prefer-btn, .ignore-btn', function (e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        var isPrefer = btn.hasClass('prefer-btn');
        var isActive = btn.hasClass('active-pref');

        var newStatus = 0;
        if (!isActive) {
            newStatus = isPrefer ? 1 : -1;
        }

        $.ajax({
            url: getPath() + "/ajax/toggle_preference/" + itemType + "/" + itemId,
            type: "POST",
            data: { status: newStatus },
            success: function () {
                var container = btn.closest('.user-pref-overlay');
                container.find('.active-pref').removeClass('active-pref');
                if (newStatus !== 0) {
                    btn.addClass('active-pref');
                }
                if (newStatus === -1) {
                    btn.closest('.book').fadeOut();
                }
            }
        });
    });
    ...
    // Event handler for search on enter
...
$(function () {
    ...
    $("#check_db_integrity").click(function () {
        $("#DialogHeader").addClass("hidden");
        $("#DialogFinished").addClass("hidden");
        $("#DialogContent").html("");
        $("#spinner2").show();
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            url: getPath() + "/ajax/integrity_check",
            success: function success(data) {
                $("#spinner2").hide();
                $("#DialogContent").html("Database check task started. Please check Tasks list for progress.");
                $("#DialogFinished").removeClass("hidden");
            }
        });
    });
