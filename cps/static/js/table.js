...
/* Function for formatting Username with Waiting badge */
function UserNameFormatter(value, row) {
    if (row.role === 0) {
        return value + ' <span class="label label-danger">Waiting</span>';
    }
    return value;
}

/* Function for deleting/approving Users */
function UserActions(value, row) {
    ...
    if (row.role === 0) {
        actions.unshift(
            "<div class=\"user-approve text-success\" style=\"cursor:pointer; display:inline-block; margin-right:10px;\" onclick=\"approveUser('" + row.id + "')\" title=\"Approve User\">",
...
function approveUser(userId) {
    $.ajax({
        ...
        error: function (data) {
            handleListServerResponse([{ type: "danger", msg: data.responseText || "Error approving user" }]);
        }
    });
}
