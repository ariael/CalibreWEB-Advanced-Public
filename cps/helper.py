...
def edit_book_read_status(book_id, read_status=None):
    if not config.config_read_column:
        ...
        if read_status is None:
            ...
            book.progress_percent = 100.0 if read_status == True else 0.0
...
def get_book_cover(book_id, resolution=None):
    # Validating permissions remains important, but we can speed up the data fetch
    # First check if user can access book (this is the permission check)
    # Ideally we'd cache this too but it depends on user.
    # We can optimize by not loading the full object if we only need the path.
    
    # Fast path: check thumbnail cache first without loading heavy book object
    if resolution:
        thumb_filename = get_book_cover_thumbnail_filename(book_id, resolution)
        if thumb_filename:
             cache = fs.FileSystem()
             if cache.get_cache_file_exists(thumb_filename, CACHE_TYPE_THUMBNAILS):
                return send_from_directory(cache.get_cache_file_dir(thumb_filename, CACHE_TYPE_THUMBNAILS),
                                           thumb_filename, max_age=604800)
    
    # If no thumbnail or resolution, fallback to loading book to get path
    # But use a cached path lookup to avoid full object load
    book_info = get_cached_book_path(book_id)
    if not book_info:
        # Book permissions fail or doesn't exist
        # Fallback to standard flow for safety or 404
        book = calibre_db.get_filtered_book(book_id, allow_show_archived=True)
        return get_book_cover_internal(book, resolution=resolution)

    path, has_cover = book_info
    
    if has_cover:
        if config.config_use_google_drive:
             # GDrive Logic remains slow/complex, keep original flow for now or Todo
             book = calibre_db.get_filtered_book(book_id, allow_show_archived=True)
             return get_book_cover_internal(book, resolution=resolution)
        else:
             cover_file_path = os.path.join(config.get_book_path(), path)
             if os.path.isfile(os.path.join(cover_file_path, "cover.jpg")):
                return send_from_directory(cover_file_path, "cover.jpg", max_age=604800)
            ...
            if thumbnail:
                ...
                if cache.get_cache_file_exists(thumbnail.filename, CACHE_TYPE_THUMBNAILS):
                    return send_from_directory(cache.get_cache_file_dir(thumbnail.filename, CACHE_TYPE_THUMBNAILS),
                                               thumbnail.filename, max_age=604800)
...
def get_book_cover_thumbnail_filename(book_id, resolution):
    thumb = (ub.session
...
def get_book_cover_thumbnail(book, resolution):
    ...
    filename = get_book_cover_thumbnail_filename(book.id, resolution)
    if filename:
        return ub.Thumbnail(filename=filename, entity_id=book.id, type=THUMBNAIL_TYPE_COVER, resolution=resolution)
    return None

@lru_cache(maxsize=4096)
def get_cached_book_path(book_id):
    # Returns (path, has_cover) tuple
    # Note: This bypasses 'get_filtered_book' permissions!
    # Users should only hit this via routes that have already checked permissions or if cover visibility is considered public.
    # In Calibre-Web, covers are generally visible to logged in users.
    row = calibre_db.session.query(db.Books.path, db.Books.has_cover).filter(db.Books.id == book_id).first()
    return (row.path, row.has_cover) if row else None
...
def get_series_cover_thumbnail(series_id, resolution=None):
    # Optimized: Try caching first
    # Currently implementation calls get_book_cover_internal on a book in the series.
    # We should cache the series->book_id mapping
    # For now, let's cache the result of the query
    
    # Can't easily cache the whole internal logic without refactoring.
    # But we can cache the "Representative Book ID" for the series.
    book_id = get_cached_series_book(series_id)
    if book_id:
        return get_book_cover(book_id, resolution)
    return get_cover_on_failure()

@lru_cache(maxsize=1024)
def get_cached_series_book(series_id):
    book = (calibre_db.session
        .query(db.Books.id)
        .join(db.books_series_link)
        .join(db.Series)
