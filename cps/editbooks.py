...
from . import constants, logger, isoLanguages, gdriveutils, uploader, helper, kobo_sync_status, audit_helper
from .clean_html import clean_string
from . import config, ub, db, calibre_db
from .services.worker import WorkerThread
from .tasks.upload import TaskUpload
from .tasks.metadata import TaskUpdateMetadata
    ...
    for elem in elements:
        ...
        try:
            if param == 'series_index':
                ...
                # Optimistic commit for better performance
                calibre_db.session.commit()
                calibre_db.clear_cache()
                WorkerThread.add(current_user.name, TaskUpdateMetadata(book.id, None))
                ...
                calibre_db.session.commit()
                calibre_db.clear_cache()
                WorkerThread.add(current_user.name, TaskUpdateMetadata(book.id, input_authors[0]))
                ret = {"success":True,
    ...
    try:
        ...
        if not upload_formats:
            ...
            if author_change or title_change:
                ...
                #                                                  config.get_book_path(),
                #                                                  input_authors[0])
                # We defer this to the background task after commit
        ...
        if config.config_use_google_drive:
            gdriveutils.updateGdriveCalibreFromLocal()
        if edit_error is not True and cover_upload_success is not False:
            if edited_books_id:
                author_arg = input_authors[0] if 'input_authors' in locals() else None
                WorkerThread.add(current_user.name, TaskUpdateMetadata(edited_books_id, author_arg))
...
def file_handling_on_upload(requested_file):
    ...
    # Proactive format/language compliance check
    if file_ext in ['docx', 'epub']:
        if not audit_helper.is_czech_content(meta.file_path, file_ext):
            flash(_("Warning: Uploaded %(ext)s file does not appear to contain Czech content and may be marked as Unhealthy.", 
                    ext=file_ext.upper()), category="warning")
...
def upload_book_formats(requested_files, book, book_id, no_cover=True):
    ...
    for requested_file in requested_files:
        ...
        if current_filename != '':
            ...
            # Proactive format/language compliance check
            if file_ext in ['docx', 'epub']:
                if not audit_helper.is_czech_content(saved_filename, file_ext):
                    flash(_("Warning: Uploaded %(ext)s file does not appear to contain Czech content and may be marked as Unhealthy.", 
                            ext=file_ext.upper()), category="warning")
