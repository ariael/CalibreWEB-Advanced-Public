...
def set_debug_tags(tags):
    global _DEBUG_TAGS
    _DEBUG_TAGS = [t.upper() for t in tags]


class _Logger(logging.Logger):
    def debug_tag(self, tag, message, *args, **kwargs):
        if tag.upper() in _DEBUG_TAGS:
            # Upgrade to INFO level if tag is enabled, so it shows up in default logs
            self.info(f"[{tag.upper()}] {message}", *args, **kwargs)
        else:
            self.debug(f"[{tag.upper()}] {message}", *args, **kwargs)
